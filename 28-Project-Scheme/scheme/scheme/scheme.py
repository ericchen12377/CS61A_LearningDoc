...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda ex: scheme_eval(ex, env))
...
def do_or_form(expressions, env):
    ...
    if not expressions:
        return False
    next = None
    while expressions.first != nil:
        next = scheme_eval(expressions.first, env)
        if is_true_primitive(next):
            return next
