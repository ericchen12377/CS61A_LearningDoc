...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while range_count < self.min_range and curr != beehive:
            curr = curr.entrance
            range_count += 1
        
        while range_count < self.max_range and not curr.bees and curr != beehive:
            curr = curr.entrance
            range_count += 1
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.real_queen:
            ...
            place_counter = self.place.exit
            while place_counter:
                if place_counter.ant:
                    if isinstance(place_counter.ant, ContainerAnt):
                        if place_counter.ant.contained_ant and place_counter.ant.contained_ant not in QueenAnt.doubled_ants:
                            place_counter.ant.contained_ant.damage *= 2
                            QueenAnt.doubled_ants.append(place_counter.ant.contained_ant)
                    if place_counter.ant not in QueenAnt.doubled_ants:
                        place_counter.ant.damage *= 2
                        QueenAnt.doubled_ants.append(place_counter.ant)
                place_counter = place_counter.exit
